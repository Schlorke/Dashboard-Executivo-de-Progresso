    ...
    properties.forEach((prop) => {
      ...
      if (value && value.includes("oklch")) {
        ...
        Object.entries(oklchToRgb).forEach(([oklch, rgb]) => {
          rgbValue = rgbValue.replace(new RegExp(oklch.replace(/[()]/g, "\\$&"), "g"), rgb)
        })
...
export default function GBLBudgetPresentation() {
  ...
  const Stat = ({ label, value, hint, delay = 0 }: { label: string; value: string; hint?: string; delay?: number }) => (
    ...
    >
      ...
      <div className="relative z-10">
        ...
        {hint && <div className="text-xs text-purple-300 opacity-80">{hint}</div>}
      </div>
  ...
  const Section = ({
    ...
    children,
    delay = 0,
  }: { title: string | React.ReactNode; children: React.ReactNode; delay?: number }) => (
    <section
  ...
  return (
    <div ref={dashboardRef} className="min-h-screen w-full bg-black relative overflow-hidden">
        ...
        >
          <div className="space-y-3">
            <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-gradient-to-r from-purple-400/15 to-violet-400/15 border border-purple-300/25 backdrop-blur-2xl">
              ...
              <span className="text-sm font-medium text-purple-200">Projeto em Andamento</span>
            </div>
            ...
            <p className="text-lg text-gray-300 font-medium">Dashboard Executivo de Progresso</p>
            <p className="text-sm text-gray-400">Atualizado em {new Date().toLocaleDateString("pt-BR")} • Versão 2.0</p>
          </div>
            ...
            <ChartContainer>
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={cumulative}>
                  <CartesianGrid strokeDasharray="3 3" opacity={0.1} />
                  <XAxis dataKey="name" tick={{ fontSize: 12, fill: "#e5e7eb" }} />
                  <YAxis tickFormatter={(v) => brl.format(v)} tick={{ fontSize: 12, fill: "#e5e7eb" }} />
                  <Tooltip
                    formatter={(v: number) => brl.format(v)}
                    contentStyle={{
                      backgroundColor: "rgba(139, 92, 246, 0.05)",
            ...
            <ChartContainer>
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={market}>
                  <defs>
                    <linearGradient id="marketGradient0" x1="0" y1="0" x2="0" y2="1">
                      <stop offset="0%" stopColor="#a78bfa" />
                      <stop offset="100%" stopColor="#8b5cf6" />
                    </linearGradient>
                    <linearGradient id="marketGradient1" x1="0" y1="0" x2="0" y2="1">
                      <stop offset="0%" stopColor="#c4b5fd" />
                      <stop offset="100%" stopColor="#a855f7" />
                    </linearGradient>
                    <linearGradient id="marketGradient2" x1="0" y1="0" x2="0" y2="1">
                      <stop offset="0%" stopColor="#a855f7" />
                      <stop offset="100%" stopColor="#9333ea" />
                    </linearGradient>
                    <linearGradient id="marketGradient3" x1="0" y1="0" x2="0" y2="1">
                  ...
                  <Bar dataKey="value" radius={[8, 8, 0, 0]}>
                    {market.map((entry, index) => (
                      <Cell key={`cell-${index}`} fill={`url(#marketGradient${index})`} />
                    ))}
                  </Bar>
                </BarChart>
              </ResponsiveContainer>
                  ...
                  <div className="relative z-10">
                    <div className="flex items-start justify-between gap-4 mb-6 pr-32">
                      <div className="space-y-2 flex-1">
                        <div className="text-sm font-bold text-purple-300 tracking-wider">{m.key}</div>
                        <h3 className="text-xl font-bold tracking-tight leading-tight text-white pr-4">{m.title}</h3>
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-3 gap-4 mb-6">
                      <div className="rounded-2xl bg-gradient-to-br from-purple-800/30 to-purple-700/20 p-4 border border-purple-400/25 backdrop-blur-2xl">
                        <div className="text-xs text-purple-200 font-medium mb-1">Recebido</div>
                        <div className="text-lg font-bold text-white">{brl.format(m.paid)}</div>
                      </div>
                      <div className="rounded-2xl bg-gradient-to-br from-violet-800/30 to-violet-700/20 p-4 border border-violet-400/25 backdrop-blur-2xl">
                        <div className="text-xs text-violet-200 font-medium mb-1">Restante</div>
                        <div className="text-lg font-bold text-white">{brl.format(Math.max(0, m.total - m.paid))}</div>
                      </div>
                      <div className="rounded-2xl bg-gradient-to-br from-purple-800/30 to-purple-800/30 p-4 border border-purple-400/25 backdrop-blur-xl">
                        <div className="text-xs text-purple-200 font-medium mb-1">Progresso</div>
                      ...
                      {m.substeps.map((s, i) => (
                        ...
                        >
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <p className="text-xs text-purple-200 leading-relaxed">{s.justification}</p>
                          </div>
